    ...
    private static LlmsDocument BuildDocument(string version)
    {
        return new LlmsDocument(
            ...
            [
                ...
                new LlmsCommandEntry("bbt pr create [-t/--title <text>] [--body <text>|--body-file <path>] [-s/--source <branch>] [-d/--destination <branch>] [-r/--reviewer <uuid|account-id>]... [--close-source-branch] [--draft]", "Create a pull request. Defaults: source = current branch (must be pushed), destination = repo main branch, title = latest commit subject on source. Output: --quiet prints the new PR id; --json emits the PR view object."),
